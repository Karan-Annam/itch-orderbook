...
# Hardware reference: pipeline stage cycle counts, modeled at 250 MHz (4 ns/cycle).
HW_NS_PER_CYCLE = 4.0
HW_STAGES = {  # cycles
    "udp_strip": 42, "itch_decode": 37, "ref_lookup": 2,
    "book_update": 4, "best_tracker": 1,
}
HW_ADD_NS = sum(HW_STAGES.values()) * HW_NS_PER_CYCLE          # ~344 ns
HW_DELETE_NS = (sum(HW_STAGES.values()) + 9) * HW_NS_PER_CYCLE  # ~380 ns
...
def main():
    ...
    sw_p = percentiles(sw)
    print("=== Software (RDTSC) vs Hardware (RTL, modeled) latency ===")
    ...
    print(f"\nHardware Add latency  ~{HW_ADD_NS:.0f} ns (deterministic)")
