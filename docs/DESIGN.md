...
array — price *is* the index — so an update is a load, an add, a store. Best
...
modeled at — and it's the *same* number every time. The FPGA story isn't that
the mean is a bit better; it's that the p99.9 equals the p50. That determinism
is what firms actually pay for. (Caveat: 250 MHz is a modeled clock, not
